...
- `supabase/migrations/0001_init.sql` — schema for the read-only Compétition tables. `0002_acces.sql` adds the `acces` allowlist table + the `verifier_acces_avant_creation()` Postgres function for the Supabase "Before user created" Auth Hook. `0001`–`0026` were applied manually via the Supabase SQL editor. **As of 27/07/2026, `npm run db:migrer` (`scripts/appliquer-migrations.js`) applies pending migrations directly via a Postgres connection (`DATABASE_URL` in `.env.local`, not committed) — Claude can run this itself now, no more manual SQL-editor copy/paste needed.** Tracked in `_migrations_appliquees` (one row per applied filename); `0001`–`0026` were backfilled into that table without re-running (already in the live schema) when the tool was introduced — see the script's header comment. Auth Hooks still need wiring in the Dashboard (Authentication → Hooks), which this tool cannot do.
...
- **Auth**: Supabase Auth, email + magic link (no passwords, no numeric code — code-entry was abandoned, see `CONTEXTE_PROJET.md` for the Brevo/DMARC saga that led here). Uses Supabase's **default mailer** (no custom SMTP) deliberately — `src/lib/supabase/client.ts` (browser), `src/lib/supabase/server.ts` (Server Components/Actions, via `next/headers` cookies), `src/proxy.ts` (Next.js 16's renamed `middleware.ts` — refreshes the session cookie every request), `src/app/auth/callback/route.ts` (PKCE code exchange after the emailed link is clicked). Login UI: `src/app/connexion/page.tsx` + `src/components/ConnexionForm.tsx`. **Keep any auth-state read out of Server Components rendered in the root layout** (i.e., out of `NavBar.tsx`) — calling `cookies()`/`getUser()` there forces every page in the tree to dynamic rendering, defeating the static-prerender speed pitch this whole project exists to prove. Auth state lives client-side instead, in `src/components/AuthNavLink.tsx` (hydrates via `onAuthStateChange`).
- **Access model**: `acces` table = login allowlist only (mirrors the original app's "Accès" sheet), not the member registry. Three role columns: `est_ca` (official board members), `est_admin` (Jérôme only — full CA-equivalent access without being an official board member, mirrors `MEMBRES_CA`'s `masque:true` entry in the original app), `masque` (excludes a row from any future "CA members" display list, access-control-irrelevant). RLS helper functions in `supabase/migrations/0003_role_ca.sql`: `est_membre_ca()` (= `est_ca OR est_admin`), `est_utilisateur_autorise()` (= has any row in `acces`, i.e. "can log in at all" — do not confuse with a real "licencié" check). True `est_licencie()`/`est_membre()` (based on `adhesions.type`, once real member data exists) are documented but not yet implemented at the end of `0004_registre_membres.sql`.
- **Logo**: `public/logo.png`, decoded once from `carreau-mondorf-app/Logo.js`'s `LOGO_BASE64`. Served as a real static file via `next/image` in `NavBar.tsx` (browser-cacheable, auto-resized) — **never inline it as base64**, that was one of the original app's known unfixed perf issues (166KB re-sent on every page load, no caching possible).
