...
### Reste en attente
- Note pense-bête #102 : sur `/national-d2`, rendre une ligne de résultat cliquable pour voir le détail du match (pas seulement éditer), ajouter une colonne points, dashboard "qui a marqué le plus de points" du jour avec bouton de bascule vers les stats. **Toujours en attente au 01/08/2026 (repassée via `/pb`, volontairement pas commencée)** — feature UI complexe à plusieurs écrans : conformément à la note de process ci-dessus (maquette avant code sur les gros chantiers UI), à prototyper en maquette avec Jérôme avant d'écrire quoi que ce soit, pas à enchaîner directement en code.
...
- **Note #106 laissée `a_traiter`** : "connectez dans l'organisation des manifestations et détaches le nom, faisons partie des membres et ou licenciés du club, et laissez la possibilité de faire une saisie libre" — transcription trop hachée pour en tirer une action fiable. Vérifié : le formulaire d'inscription à un créneau (`CreneauAffectations.tsx`) est **déjà** en saisie libre (pas de restriction à une liste de membres), donc la lecture la plus littérale de la demande semble déjà satisfaite — mais aucune autre zone de saisie liée au nom n'a été trouvée dans le module manifestations (pas de champ "organisateur/responsable" dans le schéma). Signalé à Jérôme pour préciser à quel écran/champ la note fait référence avant d'agir.
