...
- **Reverrouiller les statistiques individuelles au CA** (décision actée en Phase 1 : public "pour l'instant", à corriger maintenant que l'auth existe) — nécessite de distinguer "licencié connecté" de "membre du CA", ce que la table `acces` ne fait pas encore (juste email+nom, pas de rôle). À traiter une fois la Phase 3 vraiment validée en conditions réelles (pas encore le cas, voir blocage ci-dessus).
- Commit + push (Jérôme) ; déploiement Vercel ; validation visuelle humaine.
- **Premier réflexe à la reprise** : vérifier le statut du contact `jerome_doyen@yahoo.fr` dans Brevo (Problème A ci-dessus) avant toute autre piste.
