...
export function emailBienvenue({ prenom, email }: { prenom: string; email: string }): string {
  return `
  ...
  <tr>
    <td align="center">
      <table role="presentation" width="100%" style="max-width:480px;background:${COULEURS.sableCarte};border-radius:16px;overflow:hidden;border:1px solid ${COULEURS.ligne};">
        <tr>
          <td style="background:${COULEURS.terracotta};padding:28px 32px;text-align:center;">
        ...
        <tr>
          <td style="padding:32px;">
            ...
            <p style="margin:0 0 24px;font-size:15px;line-height:1.6;color:${COULEURS.encreDouce};">
              Tu as maintenant accès à l'espace licenciés : calendrier du club, manifestations,
              bénévolat, statistiques de compétition, et ta propre page personnelle.
            </p>
            ...
            <p style="margin:0;font-size:13px;line-height:1.6;color:${COULEURS.encreDouce};">
              Un code à 6 chiffres t'est envoyé par email à chaque connexion — aucun mot de passe à retenir.
            </p>
          </td>
        </tr>
        <tr>
