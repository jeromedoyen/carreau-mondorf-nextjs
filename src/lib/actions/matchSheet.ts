...
import { rapprocherJoueursCM, type LicencieCM } from '@/lib/rapprochementJoueurs';
 ...
 *  Rapprochement des noms (28/07/2026, demande Jérôme, "obligatoire") :
 *  "joueursCM" tel que saisi (surnom, prénom + initiale, nom + initiale…)
 *  est résolu vers la forme canonique "NOM Prénom" des licenciés du club —
 *  cf. rapprocherJoueursCM (src/lib/rapprochementJoueurs.ts), port
 *  simplifié de rapprocherNomJoueur_/canoniserJoueursCM_
 *  (ChampionnatBackend.gs) — indispensable pour que les statistiques par
 *  joueur (StatistiquesD2) agrègent bien sous un seul nom et pas une par
 *  variante orthographique. Le contrôle d'accès CA est fait explicitement
 *  ici ET implicitement par la RLS (0005_ecriture_ca.sql) — double
 *  sécurité. */
export async function enregistrerResultatRencontre(
  ...
  const { data: adhesionsData, error: errRoster } = await supabase
    .from('adhesions')
    .select('type, personnes(prenom, nom)')
    .eq('type', 'Licencié')
    .eq('supprime', false);
  if (errRoster) return { ok: false, error: errRoster.message };
  const roster: LicencieCM[] = (adhesionsData ?? [])
    .map((a) => a.personnes as unknown as LicencieCM | LicencieCM[] | null)
  ...
  const lignes = parties.map((p) => {
    ...
    return {
      ...
      type: p.type,
      ordre: p.ordre,
      joueurs_cm: rapprocherJoueursCM(p.joueursCM, roster),
