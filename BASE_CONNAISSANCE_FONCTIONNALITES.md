...
- **Onglet « Championnat »** : chaque rencontre du championnat national disputée (National D2 ou D1 selon la saison) — journée, date, camp, adversaire, score collectif, parties gagnées — et le détail de ses propres parties. Chaque ligne mène à la feuille de match.
- **Onglet « Championnat »** : en tête, la place du club au classement du championnat national (« 1er sur 7 · 22 pts · après la journée 14 »), qui mène au classement ; puis les rencontres auxquelles le licencié a pris part.
- **Onglet « Promotion »** : en tête, la place du club au dernier classement publié par la fédération (« 8e sur 14 · 315 pts · classement final ») ; puis le bilan chiffré si les résultats de la saison sont enregistrés, sinon un état vide explicite ; et la liste des journées de Promotion déclarées. Si la saison n'est couverte qu'en partie, le bilan l'indique (« 2 journées sur 10 ») avec une phrase d'explication.
...
**N'ouvre aucun accès nouveau** : `parties_d2`, `adhesions` et `personnes` sont déjà lisibles par le CA, et les statistiques de tous les joueurs figurent déjà sur `/national`.
...
### `/national` — championnat national (National D2 jusqu'en 2026, National D1 dès 2027)
...
**Ancienne adresse** : `/national-d2`, et toutes ses sous-pages, redirigent automatiquement ici — les liens déjà partagés continuent de fonctionner.
**Description détaillée** : en-tête titré de la division du club pour la saison affichée (« National D2 », « National D1 »), avec un sélecteur de saison, puis un bouton bascule "Calendrier & classement" / "Statistiques individuelles" (un seul bloc affiché à la fois, plus besoin de défiler). Le bloc calendrier montre le calendrier des rencontres du club et le classement complet de la division (7 clubs en D2, 6 en D1). Une saison dont la fédération n'a pas encore publié le calendrier l'annonce en clair, au lieu d'un tableau vide. Chaque rencontre déjà jouée (hors forfait) a une icône "œil" cliquable par tout visiteur, menant à `/national/rencontres/[id]` en consultation. Le bloc statistiques : classement complet de tous les joueurs pour le CA et la commission sportive (rôle `est_membre_commission_sportive`, migration 0044) ; pour un licencié simple, uniquement ses propres statistiques (jamais le classement des autres) ; rien pour un membre non-licencié. Chaque ligne de joueur affiche aussi le ratio points marqués/partie jouée, en plus du total de points et du taux de victoire.

### `/national/rencontres/[id]` — détail d'une rencontre
...
**Objectif** : consulter les équipes de Carreau Mondorf engagées à chaque journée de Promotion, leurs résultats, le classement des clubs, et le bilan individuel de chaque joueur.
**Description** : sélecteur de saison, saison en cours par défaut. Trois onglets : « Calendrier » (chaque trio, journée par journée, avec ses parties gagnées sur 4), « Classement » (comme pour le National : graphique d'évolution du rang des quatorze clubs journée par journée — survoler ou toucher pour choisir une journée —, classement à cette journée, puis les points de Carreau Mondorf journée par journée avec sa place ce jour-là ; aux journées où la fédération a publié un classement, c'est le sien, sinon le rang est estimé aux points et l'écran le dit) et « Statistiques » (réservé aux licenciés de la saison et au CA : classement individuel triable par taux de victoire, journées ou points ; sous chaque joueur, un panneau dépliable avec ses points par journée, toutes ses parties — adversaire et score — et ses partenaires de trio ; puis le bilan par trio). Le détail des parties vient des feuilles de journée publiées par la fédération ; une saison sans feuille importée garde son bilan par trio, et le panneau le précise. À égalité de points, les clubs sont départagés au nombre de « 4/4 » (équipes ayant gagné leurs quatre parties). Une journée dont on n'a pas la feuille est signalée par un astérisque : ses points sont déduits des classements officiels. Un club marque avec trois de ses équipes, dont au moins une mixte. Une partie arrêtée au temps (le vainqueur n'a pas 13) est notée « au temps » dans le détail d'un joueur. Le résultat est enregistré par équipe de trois : les parties gagnées du trio comptent pour chacun de ses membres. Quand la saison n'est connue qu'en partie — la fédération ne publie pas toutes les feuilles de journée —, un bandeau l'indique en tête de page (« Saison partielle — 2 journées sur 10 ») : les chiffres affichés ne portent alors que sur les journées connues. En 2026, huit journées sur dix sont connues équipe par équipe (toutes sauf la 5 et la 8).
...
**Objectif** : créer une nouvelle saison, définir laquelle est active, et indiquer dans quelle division le club joue le championnat national.
**Description** : explique que la saison active détermine ce qui s'affiche par défaut sur tout le site (compétition, manifestations, congés, membres). Formulaire de création — qui demande la division du club pour la saison (National D1 ou D2, sans choix présélectionné) — et liste des saisons existantes, où la division de chacune se lit et se corrige. Cette division donne son nom à tout le module du championnat national pour la saison.
